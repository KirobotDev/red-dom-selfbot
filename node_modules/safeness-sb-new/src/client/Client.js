...
class Client extends BaseClient {
  ...
  constructor(options) {
    ...
    // Supprimer le message sweeping obsolète
  ...
  async login(token = this.token) {
    ...
    // Nettoyer et stocker le token
    this.token = token = token.replace(/^(Bot|Bearer)\s*/i, '');
    
    // Avertissement sur l'utilisation de token utilisateur
    this.emit(
      Events.DEBUG,
    ...
    // Afficher seulement une partie du token pour des raisons de sécurité
    this.emit(
      Events.DEBUG,
      `Provided token: ${token.substring(0, 10)}...`
    );

    // Configurer la présence si définie
   ...
   * @returns {Promise<string | null>} Token of the account used
   */
  async passLogin(email, password, code = null) {
    try {
      ...
      this.emit(Events.DEBUG, `Pass login failed: ${error.message}`);
    }
  ...
  destroy() {
    ...
    // Nettoyer toutes les fonctions de cleanup
    for (const fn of this._cleanups) fn();
    this._cleanups.clear();

    // Supprimer l'intervalle de sweep de messages
    if (this.sweepMessageInterval) {
      clearInterval(this.sweepMessageInterval);
      this.sweepMessageInterval = null;
    }

    // Nettoyer les sweepers et WebSocket
  ...
  async logout() {
    try {
      ...
      this.emit(Events.DEBUG, `Logout error: ${error.message}`);
    } finally {
      this.destroy();
    }
  }
   ...
   * will be removed from the caches.
   ...
   * @param {number} timeout Time in milliseconds to sleep
   * @returns {Promise<void>}
   */
  sleep(timeout) {
  ...
  async acceptInvite(invite, options = { bypassOnboarding: true, bypassVerify: true }) {
    ...
    // Vérifier si on est déjà dans la guilde ou le canal
    ...
    if (i.guild?.id) {
      ...
      if (options.bypassOnboarding) {
        try {
          ...
          this.emit(Events.DEBUG, `[Invite > Guild ${i.guild?.id}] Onboarding bypass failed: ${error.message}`);
        }
      }
      ...
      if (data.show_verification_form && options.bypassVerify) {
        try {
          const getForm = await this.api
            .guilds(i.guild?.id)
            ['member-verification'].get({ query: { with_guild: false, invite_code: i.code } })
            .catch(() => null);
          ...
          this.emit(Events.DEBUG, `[Invite > Guild ${i.guild?.id}] Verify bypass failed: ${error.message}`);
        }
      }
   ...
   * @returns {Promise<any>}
   */
  async redeemNitro(nitro, channel, paymentSourceId) {
   ...
   * @returns {Promise<any>}
   */
  async authorizeURL(url, options = { authorize: true, permissions: '0' }) {
    const url_ = new URL(url);
    const validHosts = ['discord.com', 'canary.discord.com', 'ptb.discord.com'];
    const validPaths = [/\/api\/(v\d{1,2}\/)?oauth2\/authorize/, /\/oauth2\/authorize/];
    
    if (!validHosts.includes(url_.hostname) || !validPaths.some(pattern => pattern.test(url_.pathname))) {
   ...
   */
  async installUserApps(applicationId) {
    try {
      const rawData = await this.api.applications(applicationId).public.get({
        query: {
          with_guild: false,
        },
      });
      ...
      this.emit(Events.DEBUG, `Install user apps failed: ${error.message}`);
    }
   ...
   * @returns {Promise<void>}
   */
  async deauthorize(applicationId) {
    try {
      const tokens = await this.api.oauth2.tokens.get();
      const token = tokens.find(o => o.application.id == applicationId);
      if (token) {
        await this.api.oauth2.tokens(token.id).delete();
      }
    } catch (error) {
      this.emit(Events.DEBUG, `Deauthorize failed: ${error.message}`);
    }
  }

  /**
   * Calls eval on a script with the client as `this`.
  ...
  _validateOptions(options = this.options) {
    const validations = [
      { prop: 'makeCache', type: 'function' },
      { prop: 'messageCacheLifetime', type: 'number' },
      { prop: 'messageSweepInterval', type: 'number' },
      { prop: 'sweepers', type: 'object' },
      { prop: 'invalidRequestWarningInterval', type: 'number' },
      { prop: 'partials', type: 'array' },
      { prop: 'DMChannelVoiceStatusSync', type: 'number' },
      { prop: 'waitGuildTimeout', type: 'number' },
      { prop: 'restWsBridgeTimeout', type: 'number' },
      { prop: 'restRequestTimeout', type: 'number' },
      { prop: 'restGlobalRateLimit', type: 'number' },
      { prop: 'restSweepInterval', type: 'number' },
      { prop: 'retryLimit', type: 'number' },
      { prop: 'failIfNotExists', type: 'boolean' },
    ];

    for (const validation of validations) {
      const value = options[validation.prop];
      let isValid = false;
      ...
      if (!isValid) {
        throw new TypeError('CLIENT_INVALID_OPTION', validation.prop, `a ${validation.type}`);
      }
    }
 
    if (typeof options.rejectOnRateLimit !== 'undefined' &&
