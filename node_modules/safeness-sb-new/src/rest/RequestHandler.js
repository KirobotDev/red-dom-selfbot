  ...
  async execute(request, captchaKey, captchaToken) {
    ...
    Sublimit: ${sublimitTimeout ? `${sublimitTimeout}ms` : 'None'}`,
      ...
      try {
        ...
        if (data?.code && SILENT_ERRORS.has(data.code)) {
          this.manager.client.emit(
            DEBUG,
            `Requête ignorée (code ${data.code}): ${data.message || 'Aucun message'}
    Method  : ${request.method}
    Path    : ${request.path}
    Route   : ${request.route}
    Status  : ${res.status}`
          ...
          if (request.method === 'DELETE' && [10003, 10008].includes(data.code)) {
            return { deleted: false, reason: data.message };
          }
