  ...
  run: async (client, message, args, db) => {
    try {
      ...
      if (!message.member.permissions.has("MANAGE_WEBHOOKS"))
        return message.edit(await language(client, "Vous n'avez pas les permissions nécessaires pour utiliser cette commande", "You don't have the permissions for using this command"));
      ...
      message.edit(await language(client, `Aucun utilisateur trouvé pour \`${args[0]}\``, `No user found for \`${args[0]}\``));
    }
  }
};
