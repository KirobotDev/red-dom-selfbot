...
const macroDBPath = path.join(__dirname, "macrocmd.json");
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === 'stop') {
            ...
            if (args[1]) {
                ...
                return message.channel.send(`${listMessage}\nVeuillez choisir la macro à arrêter, exemple : \`&macrocmd stop 1\``);
            }
        }
...
\`&macrocmd (texte) (temps en s, m ou h) (id du bot)\` ☆ **Créer une macro**
\`&macrocmd liste\` ☆ **Donne la liste des macros**
\`&macrocmd stop\` ☆ **Arrête et supprime une macro**`);
        }

        const macroText = args.slice(0, -2).join(' ');
        ...
        macroIntervals[macroId] = setInterval(async () => {
            try {
                await message.channel.sendSlash(botId, macroText); 
                console.log(`Commande envoyée : ${macroText}`);
            } catch (error) {
                console.error('Erreur lors de l\'envoi de la commande :', error);
                clearInterval(macroIntervals[macroId]);
                delete macroIntervals[macroId];
                await message.channel.send('`❌` Erreur lors de l\'envoi de la commande.');
            }
        }, ms(time));

        await message.channel.send(`Macro démarrée. Répétition de "${macroText}" toutes les ${time}. Utilisez \`&macrocmd stop\` pour arrêter.`);
    }
};
